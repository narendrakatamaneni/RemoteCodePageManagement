...
public:
  //----< acquire reference to shared rcvQ >-------------------------
	...
	ClientHandler(BlockingQueue<Message>* pQ, const
		std::string& rcvPath, const std::string& name = "clientHandler") : pQ_(pQ), saveFilePath (rcvPath), clientHandlerName(name)
  {
...
void Comm::start(const std::string& rcvpath , const std::string& sndPath )
{
  BlockingQueue<Message>* pQ = rcvr.queue();
  ClientHandler* pCh = new ClientHandler(pQ,rcvpath, commName);
