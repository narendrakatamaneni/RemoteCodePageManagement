...
//----< get string between double quotes in a string >---------------------------------------

std::string Path::getValueInQuotes(const std::string& src) {
	std::string result;
	std::string line = src;

	if (std::string::npos!=line.find("/")) {

		unsigned first = src.find_last_of("/");
		unsigned last = src.length();
		result = src.substr(first+1, last-first-3);
	}
	else {
		unsigned first = src.find("\"");
		unsigned last = src.find_last_of("\"");
		result = src.substr(first + 1, last - first - 1);
	}
...
std::string Path::getValueQuotes(const std::string& src) {
	std::string result;
	std::string line = src;
...
std::string Path::getDependencyPath(const std::string& src) {
	std::string result;
	std::string line = src;
...
void Path::findAndReplaceAll(std::string & data, std::string toSearch, std::string replaceStr)
{
